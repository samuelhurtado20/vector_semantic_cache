# US-006: History Endpoint (`GET /preguntas`)

## 1. Story Understanding (What, Why, and What For)
- **What:** Create a `GET /preguntas` endpoint that retrieves all stored query-response records from SQLite.
...
|---|---|---|
| **Database** | Yes | Read all rows from the `cache_interacciones` table. |
...
- [ ] **Task 1: Define History Response Schema**
  - In [schemas.py](file:///c:/Users/Usuario/Documents/git_repositories/vector_semantic_cache/schemas.py), define `InteraccionHistorial`:
    - `id: int`
    - `pregunta: str`
    - `respuesta: str`
    - `fecha_creacion: datetime`
- [ ] **Task 2: Implement `/preguntas` Endpoint**
  - In [main.py](file:///c:/Users/Usuario/Documents/git_repositories/vector_semantic_cache/main.py):
    - Define a `GET /preguntas` endpoint returning a list of `InteraccionHistorial`.
    - Query the database to retrieve all rows.
    - Validate and serialize the records into the `InteraccionHistorial` schemas (automatically excluding `embedding`).
...
|---|---|---|---|
| **UC-6.1: Retrieve Entire History** | AC-1: Endpoint returns a list of all stored interactions. | DB, BE | Seed the database with 2 mock interactions. Call `GET /preguntas` and verify the array contains both records. |
...
| **UC-6.3: Check Field Output** | AC-3: Returns fields: `id`, `pregunta`, `respuesta`, `fecha_creacion`. | BE | Inspect the return JSON keys of each history item to confirm they contain `id`, `pregunta`, `respuesta`, and `fecha_creacion`. |
